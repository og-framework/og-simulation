...
| `SimulationNetSync`'s sixteen private `emit*` helpers | **OUT** | different verb, same role — they format the Warning-level window lines this initiative reads; deleting them deletes shipped output |
...
| `emit*` | format + write a shipped diagnostic line | `SimulationNetSync`'s sixteen private helpers (RN-10/RN-11 and earlier) | production write site — never grouped; **reuse this verb for any future extraction in that file, do not coin `report*`/`log*`/`diag*`** |
...
| `SimulationNetSync` (`SimulationNetSync.h`) | **landed, task 59** | `relayReadProbe()`, `relayArrivalProbe()`, `correctionVerdictProbe()`, `correctionLandingProbe()` (RN-9) — const-only, no `MutableDiagnostics` |
