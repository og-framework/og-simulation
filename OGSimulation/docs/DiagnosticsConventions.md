...
| `SimulationInputResolution` (`SimulationInputResolution.h`) | **landed, item 86 (scaffold); promoted to its own view's final home, item 87** | `relayReadProbe()` (RN-9's fourth probe) — const-only, one-member view, same "a convention with unexplained exceptions is worse than a view with one member" reasoning RN-7 already established for `SimulationReconciliation`'s view. Delegates into `InputResolutionTelemetry`'s own const accessor; reached as `inputResolution.getDiagnostics().relayReadProbe()` — no NetSync hop |
...
remove. (`SimulationInputResolution`'s view is the second instance of this ruling, after
`SimulationReconciliation`'s.)
