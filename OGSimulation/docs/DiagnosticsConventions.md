...
| `ClientPredictionClock` (`ClientPredictionClock.h`) | **landed with the input-history display's rate marks; the state it reads moved behind `EventSeamDiagnostics` afterwards** | the seven event-seam reads — `skipCount`, `lastSkipTick`, `stallCount`, `lastStallTick`, `hardResyncCount`, `lastHardResyncFromTick`, `lastHardResyncToTick` — plus `scribbleEventCountersForFenceTest` on `editDiagnostics()`. **The class's FIRST view**, landing with seven members under the same ruling that allowed reconciliation's to land with one. **What §1 means here:** those seven member names carry no `get` prefix and no *Diagnostic* infix, and none of them changed when the state moved — what changed is where the words live. They are no longer seven declarations sitting beside `m_predictionTick` on the clock's own member list, where clock logic could reach one by name; they are the fields of a single `EventSeamDiagnostics` member, so naming one from clock logic now costs a visible, greppable `m_eventSeamDiagnostics.` and the diagnostic-only boundary is structural rather than asserted by a comment beside the declarations. §1's infix ban governs the member names reached THROUGH a view, so it never reached the type that holds them; `EventSeamDiagnostics` takes the `<Subject>Diagnostics` shape `ResimSweepDiagnostics` already uses, and there is no *Diagnostic*-prefixed type in this submodule to follow instead. **The view stayed a view:** `Diagnostics` still holds a `const ClientPredictionClock&` and `getDiagnostics()` still returns it BY VALUE. Making the view the owner was considered and rejected — kept by value it would become a copy of seven words, which is still seven non-atomic loads and buys no atomicity, while moving the write-order discipline below into memberwise copy order where nothing names it. The seven words are written PER EVENT inside `advancePrediction`'s branches — the skip branch, BOTH stall branches (tier debt and graduated), and the hard-resync branch — and are read by nothing in production; each branch stores its tick before its count while the game-thread reader takes the count before the tick, so a reader that sees an incremented count is reading a tick that was written earlier still: a fresh count beside a stale tick is unreachable, and the one tear that is reachable is a stale count beside a fresh tick — the poll misses that event for one frame and picks it up cleanly at the next, never placing a mark at a tick that predates the event. Those WRITES stay on the production path and are fenced from grouping by §2's owner test exactly as `note*` is: they feed an instrument, and only the reads and the test-only mutator moved onto the view. The independence is machine-checked by a fence case named at `EventSeamDiagnostics`'s own declaration, on the model of `scribbleStateProvenanceForFenceTest`'s |
