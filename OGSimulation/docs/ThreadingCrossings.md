...
| 11 | Storage exposure at registration/teardown (`storage.add` / `storage.remove`) | **Both directions named.** GT write — `storage.add` via `USimmableUpdateComponent::tryRegisterWithNewFramework` (`GetTimerManager().SetTimerForNextTick`) at registration, `storage.remove` via the unregister facade at teardown — ↔ PT read/iterate, the `forEachSimulatable`-driven sweeps: `allocateFrontierSlotsAll`, `postPredictionAll`, `collectInputAll` | **Data — accepted debt, UNCHANGED; its FATAL consequence removed, and a MARSHALLING POINT now exists but is DELIBERATELY UNADOPTED.** Unsynchronized; no lock protects `m_storage`/cache/queueMap across the boundary. og-netcode-v2-field-defects task 3 changed three things and left this row's mechanism alone. (1) The ORDER: the client `registerSimulatable` overload publishes to storage LAST and `registerLocalCharacter` creates the delay line BEFORE the provider, so a sweep that reaches an id finds every container that id's branch will ask for — the window is closed for the invariant that actually crashed. (2) The CONSEQUENCE: `collectInputForCharacter` and `collectResimInputForCharacter` look the delay line up with `find` and answer the injected neutral on a miss, behind a `NOCACHE` warning rate-limited to one line per tick, so a future tear costs one logged tick instead of the process — it used to be a `std::out_of_range` thrown out of a Chaos step on a shipped client. (3) A marshalling point, `DeferredLifecycleQueue`: the game thread enqueues a lifecycle job, the physics thread applies it between steps. ⛔⛔ **NO ADAPTER IN THIS TREE USES THE QUEUE YET, AND THAT IS A RULING, NOT AN OMISSION.** It was implemented, wired into og-brawler-unreal, measured and then HELD, because adopting it INVERTS this row instead of closing it: it makes the physics thread the sole writer, which moves the exposure onto the GAME-thread readers — and two of those ITERATE rather than look up. `SimulationNetSync::sendCorrectionAll` walks `m_authorityWriters` and `sendLocalInputToAuthorityAll` walks `m_localInputSenders`, both once per tick from `onPostSimulationGameThread`, and an insert under a live iteration is a worse failure than the `find` it would replace. Eight more game-thread readers sit behind them: the adapter's per-frame visualization handoff, its HUD accessors, its input-history poll (`findPendingInputQueue`, the local-input-cache read), the by-id relay and remote-move doors reached from OnRep/RPC, and the systems-roster notify. Before the move all of those were same-thread with the only writer. **Adopting the queue therefore requires marshalling or snapshotting the game-thread send path FIRST**; until then this row stands as written and the queue ships unused, with its semantics pinned by tests in og-simulation-tests. ⚠ The core could not enforce adoption in any case: `registerSimulatable` is a free function any consumer may call from any thread, and nothing in og-simulation checks which one. Not newly found — audited and priced by item 94's D-2 (`DesignInputResolutionPeer.md`, "D-2, AUDITED AND REJECTED AT ITEM 94"), whose rejection of a proposed capture-side pairing check rests entirely on this crossing; that section is the analysis of record for this row, not a duplicate of it. **The window is wider than a single-callback race**: the two PT sweeps run in **two separate Chaos callbacks** — `OnPreSimulate_Internal` (→ `onGameSimulationPrediction`, sweep 1 + `allocateFrontierSlotsAll`) and `OnPostSolve_Internal` (`postPredictionAll`, `SimulationManagerUImpl.cpp:282,308`) — with the actual physics solve running between them, so a GT registration or teardown can land anywhere across that whole span, not just inside one callback | `SimulationNetSync.h`, the client `registerSimulatable` overload (the "PUBLISH LAST — createCacheFor BEFORE storage.add" guard) and the `unregisterSimulatable` facade (item-94 Part F's re-checked mirrored-invariant paragraph); local text stays authoritative, this row indexes it, not the other way around |
